...
def main_dashboard():
    ...
    if page == "Overview":
        ...
        st.dataframe(
            ...
            .apply(lambda x: ['background-color: ' + px.colors.sequential.YlGn[min(8, max(0, int(pd.to_numeric(x.iloc[i], errors='coerce')/4 * 9) if x.name == 'gpa' else int(pd.to_numeric(x.iloc[i], errors='coerce')/100 * 9)))]
                            if x.name in ['gpa', 'attendance_percentage'] and pd.notnull(x.iloc[i]) and pd.to_numeric(x.iloc[i], errors='coerce') is not None
                            else '' 
